...
if (isset($_POST['updateLR'])) {
    ...
    if (update($_POST, $_SESSION['id_updateLR']) > 0) {
        ...
        ";
        // header("Location: tables.php?id_LR=$id_tampilLR&u_berhasil=berhasil");
    } else {
        // echo "
        // <script>
        // var id = " . json_encode($id_tampilLR) . ";
...
<div class="modal fade" id="update-dataLR" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    ...
    <div class="col-6"> 
        <input type="number" class="form-control manualInput col-lg w-50 ac" id="tahun" name="tahun" value="<?= $_SESSION['update_tahun_LR'] ?>" autocomplete="off">
    </div>
</div>
...
<div class="col-6">   
    <input type="number" class="form-control manualInput w-50 ac" id="<?= $bulan[$i]?>" name="dataM[]" value="<?= $_SESSION['update_data_bulanLR'][$i]?>"
></div>
</div>       
<?php endfor; ?> 
</div>   
...
<div class="modal-footer">
  ...
  <button type="submit"  class="btn btn-primary" name="updateLR">Update</button>
</div>
</form>
</div>
</div>

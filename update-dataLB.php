...
if (isset($_POST['updateLB'])) {
    ...
    if (update($_POST, $_SESSION['id_updateLB']) > 0) {
        ...
        ";
        // header("Location: tables.php?id_LB=$id_tampilLB&u_berhasil=berhasil");
    } else {
        // echo "
        // <script>
        // var id = " . json_encode($id_tampilLB) . ";
...
<div class="modal fade" id="update-dataLB" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    ...
    <div class="col-6"> 
        <input type="number" class="form-control manualInput col-lg w-50 ac" id="tahun" name="tahun" value="<?= $_SESSION['update_tahun_LB'] ?>" autocomplete="off">
    </div>
</div>
...
<div class="col-6">   
    <input type="number" class="form-control manualInput w-50 ac" id="<?= $bulan[$i]?>" name="dataM[]" value="<?= $_SESSION['update_data_bulanLB'][$i]?>"
></div>
</div>       
<?php endfor; ?> 
</div>   
...
<div class="modal-footer">
  ...
  <button type="submit"  class="btn btn-primary" name="updateLB">Update</button>
</div>
</form>
</div>
</div>
